...
nt=15;                   %Define number of MC points on the emission surface
...
ntcvec=linspace(1,nt,nt);
emitpercent=zeros(size(ntcvec));
...
for i=1:nt
    ...
    emitpercent(i)=(length(find(ismembertol(LGEMat,0)~=0))/(59*59*211))*100;
    ntcount=ntcount+1; 
